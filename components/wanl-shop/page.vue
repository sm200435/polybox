	...
	<scroll-view class="wanlshop-page-container" :style="{
		...
		<view v-if="pageModules" :style="{
			...
			<view v-for="(item, index) in pageModules.item" :key="index">
				...
				<uni-home-btn v-if="item.type=='test'"></uni-home-btn>
				<!-- 优惠券 -->
				<uni-coupons v-if="item.type =='coupon'" :couponsList="item.data"></uni-coupons>
...
<script>
	import unicoupons from '@/components/uni-coupons/uni-coupons.vue'
	import unihomebtn from '@/components/uni-home-btn/uni-home-btn.vue'
	export default {
		name: "WanlShopPage",
		props: {
			windowHeight: {
