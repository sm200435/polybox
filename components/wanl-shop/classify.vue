		...
		<block v-else-if="categoryStyle === 3">
			<view class="wanlshop-classify-container__list">
				<scroll-view scroll-y scroll-with-animation class="left" :scroll-top="scrollTop" :style="{ height: `${mainHeight}px` }">
					<view v-for="(item,index) in categoryMenu" :key="item.id" class="item"
				...
				<block>
					<scroll-view scroll-y class="right" :style="{ height: `${mainHeight}px` }">
						<view class="list-cat">
							...
							<image lazy-load :src="$wanlshop.oss(categoryMenu[currentTab].image, 60, 60)" style="border-radius: 13rpx;" class="list-cat-banner" />
							<view class="padding-top-sm list-item">
								<!-- <view class="list-box" v-for="(category, infokey) in categoryCurrentTab"
									:key="category.id" @tap="handleProduct(category.category.id, category.category.name)"> -->
								<view class="list-box" v-for="(category, infokey) in categoryData" :key="category.id">
									...
									<view class="list-content">
										...
										<view class="content-price text-lg">
											¥{{ category.price }}
											<!-- <view class="content-add-btn" @tap.stop="handleAddProduct(category)">
												<text class="add-text">+</text>
											</view> -->
											<view class="text-sm">
												<image class="addcart" src="../../static/images/user/jiahao.png" @tap.stop="handleAddProduct(category)"></image>
											</view>
										</view>
									</view>
								</view>
...
<script>
	export default {
		...
		methods: {
			gogoods(goodsid){
				this.$wanlshop.to(
					'/pages/product/goods?id='+goodsid
				);
			},
			// 点击标题切换当前页时改变样式
			handleNav(e) {
