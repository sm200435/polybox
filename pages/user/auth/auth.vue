	...
	<view>
		<view class="auth">
			<view class="wanl-title">
				<image style="width: 620rpx;height: 203rpx;" mode="aspectFit"  src="https://www.duomeihe.shop/uploads/20230327/test1.png"></image>
			</view>
			<form @submit="formSubmit">
				...
				<view class="auth-button flex flex-direction">
					...
					<button type='warn' style="width: 100%;border-radius: 40rpx;height: 80rpx;line-height: 80rpx; background-color: #F31064;font-size: 30rpx;"  open-type="getPhoneNumber" @getphonenumber="decryptPhoneNumber">微信授权登录</button>
					<!-- #endif -->
				</view>
				<!-- 同意服务条款 -->
	...
	export default {
		...
		methods: {
					...
					success: (loginRes) => {
						this.$api.post({
							...
							success: res => {
								uni.hideLoading();
								if (res.binding == 0) {
									this.$wanlshop.to(`/pages/user/auth/perfect?token=${res.token}&platform=${provider.platform}&url=${this.pageroute}`);
								}else{
									this.$store.dispatch('user/login', res);
									this.$store.dispatch('cart/login');
									this.$store.dispatch('chat/get');
			...
			decryptPhoneNumber(e){
				...
				// 				uni.hideLoading();
				// 				if (res.binding == 0) {
				// 					that.$wanlshop.to(`/pages/user/auth/perfect?token=${res.token}&platform='mp_weixin'&url=${that.pageroute}`);
				// 				}else{
				// 					that.$store.dispatch('user/login', res);
				// 					that.$store.dispatch('cart/login');
				// 					that.$store.dispatch('chat/get');
				// 					// uni.reLaunch({url: decodeURIComponent(that.pageroute)});
				// 					uni.navigateBack()
				// 				}
				// 			}
				// 		});
