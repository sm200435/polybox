<template>
	<view>
		...
		<view class="wanlian cu-bar tabbar foot flex flex-direction">
			<button class="cu-btn wanl-bg-orange lg" style="margin: 0 auto;" @tap="addData">确认</button>
		</view>
	</view>
</template>
	...
	onLoad(option) {
		...
		if(this.goods_number == 1 || this.freight_type == 2){
			...
			if(parseInt(data.discount_price) > 0){
				info += `，其中总额￥${this.$wanlshop.bcmul(data.goods.price, data.goods.number)}，优惠￥${data.goods.discount_price}`;
			}
			if(parseInt(data.goods.freight_price) > 0){
				info += `，包含运费￥${data.goods.freight_price}`;
			}
