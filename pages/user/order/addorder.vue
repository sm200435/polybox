		...
		<view class="wanl-order"> 
			...
			<view v-for="(shop, index) in orderData.lists" :key="index" class="lists bg-white margin-bj radius-bock">
				...
				<form>
					...
					<view class="cu-form-group align-start">
						...
						<textarea maxlength="100" v-model="shop.remarks" placeholder="订单备注可选"></textarea>
					</view>
				</form>
						...
						<view class="wanl-modal">
							<view class="head padding-bj">
								<view class="content" style="justify-content: space-around;">
									<view class="text-lg" :class="couponsStatus==0 ? 'text-lg-chose' : ''" @click="couponsStatus=0">可用优惠券</view>
									<view class="text-lg" :class="couponsStatus==1 ? 'text-lg-chose' : ''" @click="couponsStatus=1">不可用优惠券</view>
								</view>
								<view class="close wlIcon-31guanbi" @tap="hideModal"></view>
							</view>
							<scroll-view class="wanl-coupon scroll-y" scroll-y="true" v-if="couponIndex != null&&couponsStatus==0">
								<view v-for="(coupon, index) in orderData.lists[couponIndex].coupon" :key="index" :class="coupon.type" style="height: auto;" class="item margin-bottom-bj radius-bock">
									...
									<view class="item-right padding-bj" style="height: auto;">
										...
										<view class="title">
											...
											<text class="text-cut wanl-gray text-min" style="color: #141414;font-size: 35rpx;font-weight: bold;">{{coupon.name}}</text>
										</view>
										<view class="content text-min" style="padding-top: 15rpx;">
											<view class="wanl-gray">
												...
												<view v-else-if="coupon.pretype == 'appoint' && coupon.validity != 0">
													<text></text>{{$wanlshop.timeFormat(coupon.createtime + coupon.validity * 86400)}} 前有效
												</view>
												<view v-else>
										...
										<view class="content text-min">
											...
											<view class="cu-btn sm round line-colour" style="background: rgba(243, 16, 100, 0.14);color: #F31064;" @tap="onCoupons(index)" >
							...
							<scroll-view class="wanl-coupon scroll-y" scroll-y="true" v-if="couponIndex != null&&couponsStatus==1">
								<view v-for="(coupon, index) in orderData.lists[couponIndex].coupon_no" :key="index" :class="coupon.type" style="height: auto;" class="item margin-bottom-bj radius-bock">
									<!-- <image :src="$wanlshop.appstc('/coupon/bg_coupon_3x.png')" class="coupon-bg"></image> -->
									<!-- <image :src="$wanlshop.appstc('/coupon/img_couponcentre_received_3x.png')" class="coupon-sign" v-if="coupon.state"></image> -->
									...
									<view class="item-right padding-bj" style="height: auto;">
										...
										<view class="content text-min">
											...
											<view class="cu-btn sm round line-colour" style="background-color: #F0F0F0;">
			...
			<view class="wanlian cu-bar tabbar solid-top foot text-df">
				<view>
					...
					<view>
					<view v-if="couponData[couponIndex]" style="margin-left: 100rpx;">
						共优惠：
			...
			<uni-popup ref="popup" type="center">
				<!-- <uni-popup-dialog mode="input" message="成功消息" :duration="2000" :before-close="true" @close="close"
					@confirm="confirm"> -->
					<!-- </uni-popup-dialog> -->
					<view style="background-color: #ccc;padding: 15px;border-radius: 20rpx;">
						<view @click="$refs.popup.close()" style="text-align: right;">
							X
						</view>
						...
						<view style="display: flex;">
							<button style="margin-right: 40rpx;" @click="confirmPayment">确认支付</button>
							<button @click="quit">狠心退出</button>
						</view>
					</view>
			</uni-popup>
		</view>
