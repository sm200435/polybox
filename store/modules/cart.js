...
export default {
	...
	actions: {
		...
		// 自定义输入数量并计算价格
		async zdyBcadd({state, dispatch}, goods) {
				goods.sum = fun.bcmul(goods.number, goods.sku.price);
				let sum=0
				let num=0
				// 计算购物车总价总数
				...
				console.log('zdyBcadd-goods.sum',sum,goods.sum);
				if (goods.checked) {
					state.allnum=num
		...
		async bcadd({state, dispatch}, goods) {
			...
			if (goods.checked) {
				...
				console.log('bcadd-state-allsum',state.allsum);
			}
